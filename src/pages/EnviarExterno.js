...
function EnviarExterno() {
  ...
  return (
    <div className="card-transferencia animate-fade-in">
      {step === 1 ? (
        <>
          <header className="card-header">
            <h2>Transferencia B2B</h2>
            <p className="sub-title">Envía dinero a otras bancas</p>
          </header>

          <form onSubmit={handleRealExternalTransfer} className="form-stack">
            <div className="form-group">
              <label>Banca de Origen</label>
              <select className="input-modern" value={selectedBank} onChange={(e) => setSelectedBank(e.target.value)} required>
            ...
            {selectedBank && !isPixelMoney && (
              <div className="warning-box">⚠️ Integración no activa para este banco</div>
            )}
            ...
            <div className="form-group">
              ...
              <input
                ...
                className={`input-modern ${verificationStatus === 'valid' ? 'input-valid' : verificationStatus === 'invalid' ? 'input-invalid' : ''}`}
                required
              />
              {verificationStatus === 'valid' && <div className="validation-msg msg-valid">✅ {beneficiaryName}</div>}
              {verificationStatus === 'invalid' && <div className="validation-msg msg-invalid">❌ No encontrado</div>}
            </div>
            ...
            <div className="form-group">
              ...
              <div className="currency-input-wrapper">
                ...
                <input type="number" placeholder="0.00" value={monto} onChange={(e) => setMonto(e.target.value)} className="amount-hero" step="0.01" min="0.10" required />
              </div>
            </div>
